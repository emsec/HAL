    ...
    namespace group_by_snn_groups_fixed
    {
        bool snnGreaterThan(std::vector<u32> v1, std::vector<u32> v2, int threshold)
        {
            ...
            for (std::vector<u32>::iterator it1 = v1.begin(); it1 != v1.end(); ++it1)
            {
                ...
                if (snn >= threshold) return true;
            }
            return false;
        }

        /* snn algorithm on input groups and neighboured groups for a fixed threshold */
        std::shared_ptr<Grouping> process(const std::shared_ptr<Grouping>& state, bool successors, bool predecessors, int threshold)
        {
            ...
            for (const auto& [group_id, gates] : state->gates_of_group)
            {
                ...
                std::set_union(successor_groups.begin(), successor_groups.end(),
                                predecessor_groups.begin(), predecessor_groups.end(),
                                std::inserter(current_row, current_row.begin()));                
