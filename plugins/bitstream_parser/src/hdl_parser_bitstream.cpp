...
namespace hal
{
    namespace
    {
        // TODO replace by existing function, need to get path from main.cpp somehow
    ...
    bool HDLParserBitstream::parse(const std::filesystem::path& file_name)
    {
        ...
        std::string cmd = "python3 -mfasm2bels --connection_database " + db_path + " --db_root " + m_config.db_root_path + "/" + device + " --part " + bin_part + " --fasm_file " + fasm_path
                          + " --bit_file " + file_name.string() + " --bitread " + m_config.bitread_path + " --verilog_file " + m_v_path + " --xdc_file " + xdc_path
